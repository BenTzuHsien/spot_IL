...
class GroundingDinoFeatureExtractor(nn.Module):
    ...
    def hook_fn(self, module, input, output):
        self._features = output.detach().clone()
...
class DINOCrossAttentionMLP(nn.Module):
    ...
    def forward(self, current_images, goal_images, text_prompts):
        ...
        for cam in range(self.num_cameras):
            ...
            if torch.isnan(curr_img).any() or torch.isinf(curr_img).any():
                print(f"NaN or Inf found in current_images at camera {cam}")
            if torch.isnan(goal_img).any() or torch.isinf(goal_img).any():
                print(f"NaN or Inf found in goal_images at camera {cam}")

            # Ensure feature outputs are contiguous
            curr_feat = self.feature_extractor(curr_img, text_prompts).contiguous()
            goal_feat = self.feature_extractor(goal_img, text_prompts).contiguous()
