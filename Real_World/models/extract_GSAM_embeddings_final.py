...
from SPOT_new_SingleStep_Discredtized_DataLoader import SPOT_SingleStep_Discretized_DataLoader
import cv2
import numpy as np
...
def main():
        ...
        for current_images, goal_images, labels in tqdm(train_dataloader, desc="Extracting embeddings"):
            ...
            # --------------------------------------------------------------------------
            # Visualization
            img_bgrs = [to_bgr(current_images[0,cam]) for cam in range(current_images.shape[1])]  
            top, mid, bot = [], [], []
            for cam in range(current_images.shape[1]):
                sam_mask = curr_mask_list[cam][0]
