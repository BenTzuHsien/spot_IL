...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, query, key_value):
        ...
        # each sample carries N = 32 x 32 tokens
        # memory usage scales to B * num_heads * N
        # BIG BATCH SIZE WILL THROW CUDA OUT OF MEMORY ERROR
    
        # query and key_value: (B, C, H, W)
        B, C, H, W = query.shape

        # [Cross-Attention] query torch.Size([16, 256, 32, 32])  key_value torch.Size([16, 256, 32, 32])
        ...
        # [Cross-Attention] out  torch.Size([16, 256, 32, 32])
