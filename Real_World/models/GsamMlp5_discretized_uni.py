...
class FlashCrossAttention(nn.Module):
    ...
    def forward(self, query, key_value):
        ...
        # [Cross-Attention] query torch.Size([64, 256, 32, 128])  key_value torch.Size([64, 256, 32, 128])   
        ...
        # [Cross-Attention] attn_output torch.Size([4096, 64, 256]) attn_weights torch.Size([64, 4096, 4096])
        ...
        # [Cross-Attention] out torch.Size([64, 256, 32, 128])
...
class GsamMlp5(nn.Module):
    ...
    def forward(self, curr, goal):
        ...
        print(f"[GSAM-MLP] curr_pool {curr_feat.shape}")    
        
    
        # MLP Layers
        x = self.fc_layer1(curr_feat)
        x = self.fc_layer2(x)
        x = self.fc_layer3(x)
        x = self.fc_layer4(x)
