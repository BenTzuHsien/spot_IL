...
class DinoMlp5(nn.Module):
    ...
    def set_goal(self, goal_images):
        goal_embeddings = []
        for image in goal_images:
            dino_output = self.shared_trunk.forward_features(image)
            embedding = torch.reshape(dino_output['x_norm_patchtokens'], [-1, 16, 16, 384]).permute(0, 3, 1, 2)
            goal_embeddings.append(embedding)

        self.goal_embeddings = torch.stack(goal_embeddings).unsqueeze(0)
        self.goal_iamges = self.goal_iamges.to(device=self.fc_layer_x.weight.device, dtype=self.fc_layer_x.weight.dtype)
